...
# def op(parametros,df,cf,t,salinity,df_pvt,sw0,cum,x0,k,water_visc,pi): N,aq_radius,res_radius,aq_thickness,
# theta=parametros P3 = pd.DataFrame({'P_calculada': pi}, index=[0]) for i in range(len(df['DATE'])): Np = df[
# 'oil_prod_cum'][i] wp = df['water_prod_cum'][i] p_anterior = P3['P_calculada'][i] presion = fsolve(press, x0,
# args=( Np, wp, cf, t, salinity, df_pvt, aq_radius, res_radius, aq_thickness, aq_por, theta, k, water_visc,
# p_anterior, cum, pi, sw0, N)) x0 = presion[0] P3 = P3._append({'P_calculada': presion[0]}, ignore_index=True)
# p_nueva = P3['P_calculada'][i + 1] cw = comp_bw_nogas(p_nueva, t, salinity, unit=1) ct = cf + cw cum =
# aquifer_fetkovich(aq_radius, res_radius, aq_thickness, aq_por, ct, p_nueva, theta, k, water_visc, p_anterior, cum,
# pi) nueva_fila = pd.DataFrame({'Tank': 'tank_center', 'Pressure': 3700.00}, index=[0]) df = pd.concat([nueva_fila,
# df]).reset_index(drop=True) df.iloc[0] = df.iloc[0].fillna(0) n_total=len(df['Pressure']) error = (((df[
# 'Pressure']-P3['P_calculada'])**2).sum())/n_total return error
#
#
# df=df_ta2
#
# bnds=[(1e+6,100e+6),(0,None),(0,None),(0,None),(20,360)] result = minimize(op, x0=[N,aq_radius,res_radius,
# aq_thickness,theta],args=(df,cf,t,salinity,df_pvt,sw0,cum,x0,k,water_visc,pi),bounds=bnds) Nop,aq_radiusop,
# res_radiusop,thicknessop, thetaop = result.x
#
...
#
# P4 = pd.DataFrame({'P_calculada': 3700.00},index=[0]) for i in range(len(df_ta2['DATE'])): Np = df_ta2[
# 'oil_prod_cum'][i] wp = df_ta2['water_prod_cum'][i] p_anterior = P4['P_calculada'][i] presion = fsolve(press, x0,
# args=( Np, wp, cf, t, salinity, df_pvt, aq_radius, res_radius, aq_thickness, aq_por, theta, k, water_visc,
# p_anterior, cum, pi, sw0, N)) x0 = presion[0] P4 = P4._append({'P_calculada': presion[0]}, ignore_index=True)
# p_nueva = P4['P_calculada'][i + 1] cw = comp_bw_nogas(p_nueva, t, salinity, unit=1) ct = cf + cw cum =
# aquifer_fetkovich(aq_radius, res_radius, aq_thickness, aq_por, ct, p_nueva, theta, k, water_visc, p_anterior, cum,
# pi) %%
nueva_fila = pd.DataFrame(
    {"Date": "1987-09-01", "Pressure": 4000.00, "oil_fvf": 1.1}, index=[0]
)
