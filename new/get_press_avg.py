...
for tank,wells in tank_wells.items():
    for well in wells:
        ...
        if press_vector is not None:
            pvt = FluidModel(data_pvt=df_pvt,temperature=25)
            ...
            press_vector.data[WATER_FVF_COL] = WaterFluidModel(correlation=Bo_bw).get_rs_at_press(press_vector.data[PRESSURE_COL],220,3000,unit=1)
            press_vector.data[RS_W_COL] = WaterFluidModel(correlation=RS_bw).get_rs_at_press(press_vector.data[PRESSURE_COL],220,3000,unit=1)
