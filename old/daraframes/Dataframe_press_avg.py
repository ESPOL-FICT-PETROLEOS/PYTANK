...
df_pressure = pd.read_csv("../../pytank/resources/data_csv/pressures.csv")
...
df_prod[cols_output] = (
    df_prod.groupby(well_name_col)[cols_input].diff().fillna(df_prod[cols_input])
)

# %% Creation of a pytank dataframe grouping the cumulative fluids produced by each tank
...
df_tanks.rename(
    columns={
        oil_vol_col: oil_cum_col,
        water_vol_col: water_cum_col,
        gas_vol_col: gas_cum_col,
    },
    inplace=True,
)
...
# %% Using the interp_dates_row function to add a pytank column to the pressure Dataframe
...
# Define the names of the pytank columns to be added in the mbal dataframe, which
