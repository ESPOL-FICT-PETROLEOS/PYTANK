...
class Fluid:
    def __init__(self, production: pd.DataFrame, pressure_data: pd.DataFrame, pvt_data: pd.DataFrame):
        """
    ...
    def interpolate_pvt_info(self):
        ...
        for index, row in self.avg.iterrows():
            ...
            if len(pvt_subset) > 1:
                ...
                for column in ['Bo', 'Bg', 'GOR']:
                    pvt_value = interp_pvt_matbal(pvt_subset,'Pressure',column, pressure_datum)
